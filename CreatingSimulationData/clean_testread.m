...
fs2_matrix=[0.7, 1.0, 1.3];
fm_matrix=[0.7, 1.0, 1.3];

i = 1;
T1W_matrix=[1.7, 2.0, 2.3];
T2W_matrix=[50, 80, 110]*0.001;
T2S_matrix = [0.002, 0.004, 0.005];
...
for ii_T1W=1:num_T1W
  ...
  for ii_T2W=1:num_T2W
      ...
      for ii_T2S=1:num_T2S
          ...
          for ii_fs1=1:num_fs1
              fs1_cal=0.0006+0.0004*(ii_fs1-1);
 ...
 mtr_normal(:,ii_T1W,ii_T2W,ii_T2S, ii_fs1, ii_fs2, ii_fs5, ii_fm, ii_ksw1) = (1-sscal_normal) - (1-SS_cal_value_ref_normal);
 mtr_tumor(:,ii_T1W,ii_T2W,ii_T2S, ii_fs1, ii_fs2, ii_fs5, ii_fm, ii_ksw1) = (1-sscal_tumor)-(1- SS_cal_value_ref_tumor);
 arex_normal(:,ii_T1W,ii_T2W,ii_T2S, ii_fs1, ii_fs2, ii_fs5, ii_fm, ii_ksw1) = ((1./sscal_normal) - (1./SS_cal_value_ref_normal)).*R1W_cal_obs*(1+(fm_cal*fm));
 arex_tumor(:,ii_T1W,ii_T2W,ii_T2S, ii_fs1, ii_fs2, ii_fs5, ii_fm, ii_ksw1) = ((1./sscal_tumor) - (1./SS_cal_value_ref_tumor)).*R1W_cal_obs*(1+(fm_cal*fm));
 i = i+1;
...
matrix_input_noise = matrix_input + (0.002+0.002*(i_SNR-1))*((rand(sz)-0.5)*2);
...
params_matrix = reshape(params_matrix, [8 num_T1W*num_T2W*num_T2S*num_fs1*num_fs2*num_fs5*num_fm*num_ksw1]);

%Creating testing matrix
matrix_MTR_output1(:,:)=reshape(mtr_normal,  [length(k') num_T1W*num_T2W*num_T2S*num_fs1*num_fs2*num_fs5*num_fm*num_ksw1]);   
matrix_MTR_output2(:,:)=reshape(mtr_tumor,  [length(k') num_T1W*num_T2W*num_T2S*num_fs1*num_fs2*num_fs5*num_fm*num_ksw1]);   
matrix_MTR_output(:,:)=[matrix_MTR_output1, matrix_MTR_output2];

matrix_AREX_output1(:,:)=reshape(arex_normal,  [length(k') num_T1W*num_T2W*num_T2S*num_fs1*num_fs2*num_fs5*num_fm*num_ksw1]);   
matrix_AREX_output2(:,:)=reshape(arex_tumor,  [length(k') num_T1W*num_T2W*num_T2S*num_fs1*num_fs2*num_fs5*num_fm*num_ksw1]);   
matrix_AREX_output(:,:)=[matrix_AREX_output1, matrix_AREX_output2]; 
...
toc
 ...
 tic
     ...
     for i=1:length(matrix_input_all_noise)
        input_denoise = smoothdata(matrix_input_all_noise(:,i),'gaussian',2);
        sig=(1 - matrix_input_all(:,i));
  
        R1W_AREX=R1W_cal_matrix_output_all2(i);
        fm_AREX=fm_cal_matrix_output_all2(i);
        
        x =k.';
        beta0= [0.9, 0, 560,               0.025, -1400, 200,        0.01, -800, 600,        0.001, 600, 400,          0.02, 1400, 1200,          0.1, 0, 10000]; % initial test
        lb=[0.02, -400, 120,               0, -1600, 160,             0, -1200, 200,           0, 400, 0,                0, 1000, 400,            0, -1600, 4000]; % lower bound
        ub=[1, 400,4000,                    0.2, -1200, 1200,          0.2,-400, 2000,         0.2, 800, 600,            1, 1800, 2000,         1, 1600, 40000]; % upper bound
